...
const NavPriv = () => {
  let Links = [
    { name: 'Inicio', link: '/social/inicio' },
    { name: 'Mi Perfil?', link: '/social/miperfil' },
    { name: 'Mis Listas', link: '/social/mislistas' },
    { name: 'Quiénes somos?', link: '/social/quienessomos' },
    { name: 'Contacto', link: '/social/contacto' },
  
  
  ];

  let [openp, setOpenp] = useState(false);
  ...
  return (
    <div className='shadow-md w-full fixed top-0 left-0'>
      <div className='md:flex items-center justify-between bg-rose-300 py-4 md:px-10 px-7'>
        ...
        <div onClick={() => setOpenp(!openp)} className='text-3xl absolute right-8 top-6 cursor-pointer md:hidden'>
          <ion-icon name={openp ? 'close' : 'menu'}></ion-icon>
        </div>
